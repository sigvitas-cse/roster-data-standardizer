...
Full verifier script (single file).

- Requires GENAI_API_KEY or GOOGLE_API_KEY environment variable (strict).
- Uses google-genai to extract education + short source URL, and verifies by fetching pages.
- Checkpointing, raw responses, raw fetched pages saved.
- CLI flags:
    --batch-size N
    --assume-trusted
    --start-row N
    --only-audit
- Outputs:
    output_with_education_verified_v5.csv
    audit_unverified_v5.csv
    raw_responses_verified_v5/
    raw_pages_v5/
Logs to education_verify_v5.log
"""
